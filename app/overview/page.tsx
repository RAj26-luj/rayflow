        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {opportunities.slice(0, 4).map((opp) => (
              <div
                key={opp.id}
                className={`rounded-xl border p-4 sm:p-5 shadow-xs transition-all flex flex-col justify-between ${
                  opp.status === 'APPROVED'
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-2.5 gap-2">
                    <span className="rounded-full bg-blue-50 border border-blue-200 px-2 sm:px-2.5 py-0.5 text-[10px] sm:text-[11px] font-semibold text-blue-700 uppercase tracking-wide truncate">
                      {opp.type.replace(/_/g, ' ')}
                    </span>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {opp.status === 'APPROVED' ? (
                        <span className="text-[10px] sm:text-[11px] font-bold text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded border border-emerald-300 flex items-center gap-1">
                          <CheckCircle2 className="h-3 w-3 text-emerald-600" />
                          <span>Approved</span>
                        </span>
                      ) : opp.status === 'EXECUTED' ? (
                        <span className="text-[10px] sm:text-[11px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded border border-emerald-400 flex items-center gap-1">
                          <Zap className="h-3 w-3 text-emerald-600" />
                          <span>Live Campaign</span>
                        </span>
                      ) : opp.status === 'SIMULATED' ? (
                        <span className="text-[10px] sm:text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200">
                          Simulated
                        </span>
                      ) : (
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                    {opp.status === 'PENDING' || opp.status === 'SIMULATED' ? (
                      <>
                        ...
                        <button
                          onClick={() => handleReviewOpp(opp)}
                          className="rounded-md bg-blue-600 px-3 sm:px-3.5 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-blue-700 transition-colors flex items-center gap-1"
                        >
                          <CheckCircle2 className="h-3.5 w-3.5" />
                          <span>Approve</span>
                        </button>
                      ...
                      <>
                        <button
                          onClick={() => handleReviewOpp(opp)}
                          className="rounded-md border border-slate-200 bg-white px-2.5 sm:px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                        >
                          Details
                        </button>
                        <button
                          onClick={() => handleReviewOpp(opp)}
                          className="rounded-md bg-emerald-600 px-3 sm:px-3.5 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-emerald-700 transition-colors flex items-center gap-1"
                        >
